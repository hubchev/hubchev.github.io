...
\date{Last compiled on 12 April, 2023}
...
\tightlist
\item
  \ldots forget: R is \textbf{case sensitive}.
\item
  \ldots name of objects such as vectors, numbers, variables, and data frames with a starting number.
\item
  \ldots use dots in names of objects.
\item
  \ldots use the following keywords in naming objects: if , else , repeat , while , function , for , in , next , break , TRUE , FALSE , NULL , Inf , NaN ,
  NA.
\item
  \ldots use the back slash \texttt{\textbackslash{}} but the front slash \texttt{/} for navigating the file system.
\item
  \ldots use whitespace and indentation for naming files, directories, or objects.
\item
  \ldots ignore warnings or errors unless you know what they mean.
\item
  \ldots use hard-coded values. Instead, define objects to represent these values.
\item
  \ldots forget to (install and) load packages that contains functions you want to use.
\item
  \ldots forget to set your working directory.
\item
  \ldots forget to comment your code.
\item
  \ldots use =, for assignment but \textless-.
\end{itemize}
